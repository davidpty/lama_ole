...
├── chat.py                  # ChatState + REPL with slash commands (/feed, /new, /model, /skill, etc.)
...
| **`chat.py`** | Interactive REPL (`ChatState`). Manages multi-turn conversation history, slash commands (`/feed`, `/new`, `/model`, `/save`, `/load`, `/tools` (load/unload/show toolsets), `/skill`, `/systemprompt`, `/context`, `/help`, `/exit`), and delegates each turn to `run_with_tools()`. |
